...
public class ClothSimulation : Simulation
{
	...
	protected override void Start()
	{
		...
		for (int y = 0; y < numPoints.y; y++)
		{
			...
			for (int x = 0; x < numPoints.x; x++)
			{
				...
				points.Add(new Point() { pos = position, posOld = position, isLocked = locked });
			}
		}

		float length = (points[0].pos - points[1].pos).magnitude;
	...
	void Cut(Vector2 start, Vector2 end)
	{
		...
		{
			if (LineSegmentsIntersect(start, end, sticks[i].pointA.pos, sticks[i].pointB.pos))
			{
				sticks[i].dead = true;
			}
		}
